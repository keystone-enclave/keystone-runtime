...
extern char *__malloc_start_rt;
extern char *__kernel_stack_start;
//extern char *__malloc_zone_stop; /* dkohlbre: added since our malloc region is static */

/* This is the minimum gap allowed between __malloc_end_rt and the top of
   the stack.  This is only checked for when __malloc_end_rt is
...
#define CAN_ALLOC_P(required)				\
  (((size_t) __malloc_end_rt - (size_t)MALLOC_LIMIT	\
    - MALLOC_MINIMUM_GAP) >= (required))
#else
#define CAN_ALLOC_P(required)				\
  (((size_t)MALLOC_LIMIT - (size_t) __malloc_end_rt	\
    - MALLOC_MINIMUM_GAP) >= (required))
#endif
...
void check_range_and_call_sbrk(void *addr, size_t sz)
{
  ...
  unsigned long cur_sbrk= rt_handle_sbrk_rt(0);cur_sbrk=cur_sbrk;
  //ocall(OCALL_PRINT_VALUE, &cur_sbrk, sizeof(unsigned long), 0, 0,0);

  if( ( (unsigned long)addr + (unsigned long)sz) >cur_sbrk  )
  {
    rt_handle_sbrk_rt(    ( (unsigned long)addr + (unsigned long)sz)    -  cur_sbrk   );
  }



  //printf("malloc start = 0x%lx and malloc end = 0x%lx and malloc chunk addr = 0x%lx \n",(unsigned long)__malloc_start_rt, (unsigned long)__malloc_end_rt, (unsigned long)addr );
}
...
malloc (size_t sz)
{
  ...
  //printf("[MALLOC]STACK POINTER = 0x%lx\n",MALLOC_LIMIT );
  ...
  if(__malloc_end_rt == NULL){
    __malloc_end_rt = &__malloc_start_rt;
  }
  ...
  for (nextfree = &__malloc_freelist;
      ...
      if (block->size >= real_size)
	{
	  ...
	  if (block->size < real_size + sizeof (struct freelist_entry))
        ...
        void* malloc_return_addr = (void *)&block->next;
        //printf("\n[MALLOC]MALLOC START = 0x%lx\n",__malloc_start_rt );
        //printf("\n[MALLOC]MALLOC START = 0x%lx\n",__malloc_end_rt );
        //printf("\n[MALLOC]KERNEL STACK START = 0x%lx\n",__kernel_stack_start );
	 ...
	 enlarge it.  */
      if (! block->next
	  && __malloc_end_rt == (void *)((size_t)block + block->size))
	{
	  ...
	  *nextfree = NULL;
	  if (MALLOC_DIRECTION < 0)
	    {
	      block = __malloc_end_rt = (void *)((size_t)block - moresize);
	    }
	  else
	    {
	      __malloc_end_rt = (void *)((size_t)block + real_size);
	    }
  ...
  if (MALLOC_DIRECTION > 0)
    {
      block = __malloc_end_rt;
      __malloc_end_rt = (void *)((size_t)__malloc_end_rt + real_size);
    }
  else
    {
      block = __malloc_end_rt = (void *)((size_t)__malloc_end_rt - real_size);
    }
 done:
...
memalign (size_t align, size_t sz)
{
  ...
  for (nextfree = &__malloc_freelist;
      ...
      if (! *nextfree)
	{
	  ...
	  if (MALLOC_DIRECTION < 0)
	    {
	      old_size += M_ALIGN_SUB (((size_t)__malloc_end_rt
					- old_size + sizeof (size_t)),
				       align);
	      if (! CAN_ALLOC_P (old_size))
		return NULL;
	      block = __malloc_end_rt = (void *)((size_t)__malloc_end_rt - old_size);
	    }
	  else
	    {
	      block = __malloc_end_rt;
	      old_size += M_ALIGN ((size_t)__malloc_end_rt + sizeof (size_t),
				   align);
	      if (! CAN_ALLOC_P (old_size))
		return NULL;
	      __malloc_end_rt = (void *)((size_t)__malloc_end_rt + old_size);
	    }
	  *nextfree = block;
	  block->size = old_size;
	  block->next = NULL;
	  ...
	  && __malloc_end_rt == (void *)((size_t)block + block->size))
	{
	  if (MALLOC_DIRECTION < 0)
		...
		return NULL;
	      __malloc_end_rt = (void *)((size_t)block + before_size + real_size);
...
mallinfo (void)
{
  ...
  for (fr = __malloc_freelist; fr; fr = fr->next)
      ...
      if (! fr->next)
	{
	  int atend;
	  if (MALLOC_DIRECTION > 0)
	    atend = (void *)((size_t)fr + fr->size) == __malloc_end_rt;
	  else
	    atend = (void *)fr == __malloc_end_rt;
	  if (atend)
	    r.keepcost = fr->size;
	}
    }
  ...
  if (MALLOC_DIRECTION > 0)
    total_size = (char *)__malloc_end_rt - (char *)&__malloc_start_rt;
  else
    total_size = (char *)&__malloc_start_rt - (char *)__malloc_end_rt;
...
malloc_stats(void)
{
  ...
  fprintf (fp, "malloc has reserved %u bytes between %p and %p\n",
	   i.arena, &__malloc_start_rt, __malloc_end_rt);
